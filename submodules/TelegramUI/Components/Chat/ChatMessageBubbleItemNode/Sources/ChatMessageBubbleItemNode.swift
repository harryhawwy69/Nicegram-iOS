...
public class ChatMessageBubbleItemNode: ChatMessageItemView, ChatMessagePreviewItemNode {
    ...
    private static func beginLayout(
        ...
        let isSecretChat = item.message.id.peerId.namespace == Namespaces.Peer.SecretChat
        
        if !isFailed || Namespaces.Message.allScheduled.contains(item.message.id.namespace) {
            if wantTrButton[0].0 {
                if item.message.id.peerId != item.context.account.peerId {
                    let textToTranslate = item.message.textToTranslate()
                    if !isSecretChat,
                       !textToTranslate.isEmpty {
                        needTrButton = true
                        if wantTrButton[0].1.isEmpty {}
                        else if #available(iOS 12.0, *) {
                            if let msgLang = detectedLanguage(for: textToTranslate) {
    ...
    private static func applyLayout(selfReference: Weak<ChatMessageBubbleItemNode>,
        ...
        if needsTrButton {
            if strongSelf.trButtonNode == nil {
                ...
                trButtonNode.view.addGestureRecognizer(
                    UITapGestureRecognizer(
                        target: strongSelf,
                        action: #selector(strongSelf.trButtonPressed)
                    )
                )
        ...
        if case .System = animation/*, !strongSelf.mainContextSourceNode.isExtractedToContextPreview*/ {
            ...
            // Nicegram Translate
            if let trButtonNode = strongSelf.trButtonNode {
                let buttonSize = trButtonNode.update(presentationData: item.presentationData, controllerInteraction: item.controllerInteraction, chatLocation: item.chatLocation, subject: item.associatedData.subject, message: EngineMessage(item.message), accountPeerId: item.context.account.peerId, disableComments: disablesComments, translateButton: true)
                ...
                animation.animator.updateAlpha(layer: trButtonNode.layer, alpha: isCurrentlyPlayingMedia ? 0.0 : 1.0, completion: nil)
            ...
            if let trButtonNode = strongSelf.trButtonNode {
                ...
                animation.animator.updatePosition(layer: trButtonNode.layer, position: buttonFrame.center, completion: nil)
                animation.animator.updateBounds(layer: trButtonNode.layer, bounds: CGRect(origin: CGPoint(), size: buttonFrame.size), completion: nil)
                animation.animator.updateAlpha(layer: trButtonNode.layer, alpha: (isCurrentlyPlayingMedia || isSidePanelOpen) ? 0.0 : 1.0, completion: nil)
                animation.animator.updateScale(layer: trButtonNode.layer, scale: (isCurrentlyPlayingMedia || isSidePanelOpen) ? 0.001 : 1.0, completion: nil)
