...
public final class ChatControllerImpl: TelegramBaseController, ChatController, GalleryHiddenMediaTarget, UIDropInteractionDelegate {    
    ...
    public init(
        ...
        let controllerInteraction = ChatControllerInteraction(onTranslateButtonLongTap: { [weak self] in
            ...
            let c = languageListController(context: self.context, selectedLanguageCode: selectedLanguageCode, selectLanguage: { code in
                setPreferredTranslationTargetLanguage(code: code)
            })
            c.navigationPresentation = .modal
            self.push(c)
        }, openMessage: { [weak self] message, params in
    ...
    func updateAiOverlayVisibility() {
        ...
        self.chatDisplayNode.nicegramOverlayNode.isHidden = self.shouldDisplayDownButton || recordingMediaMessage || displayMentionButton || displayReactionButton
    }
    //
    ...
    func replyPrivately(message: Message) {
        ...
        |> deliverOnMainQueue).start(completed: { [weak self] in
            ...
            let chatController = ChatControllerImpl(context: self.context, chatLocation: .peer(id: peerId))
            self.effectiveNavigationController?.pushViewController(chatController)
        })
    }
    //
