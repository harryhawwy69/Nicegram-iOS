---
paths:
  - "Nicegram/**"
---

# Nicegram in-repo modules (`Nicegram/**`)

`Nicegram/` holds our own `NG*` modules that live inside this repo. It predates
`nicegram-assistant-ios`, so **new standalone features should go to the assistant,
not here.**

## When to add code here

- Prefer `nicegram-assistant-ios` for new self-contained features.
- Add here only for code that must sit next to Telegram — in practice **`NGUtils`**:
  utilities over Telegram types (entity mapping, media helpers, formatting) and
  **`TelegramBridge` implementations** the assistant consumes (see
  `NICEGRAM-AGENTS.md`).
- Only create a new `NG*` module with strong reason — extend `NGUtils` or an
  existing module first.
- Each module is a Bazel/BUCK target (`BUILD`/`BUCK`): add new files and deps to
  it. Follow `swift-conventions` for style, and bridge any Telegram `Signal` at
  the edge (see `telegram-interop`).

## Resources — do not grow them here

Do **not** add new strings/images/files. Add resources to
`nicegram-assistant-ios` (which generates a typed accessor you reference).
`Nicegram/NGResources` is legacy — don't add to it.
